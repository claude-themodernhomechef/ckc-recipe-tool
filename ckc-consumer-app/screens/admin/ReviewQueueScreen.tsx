...
function cleanForDisplay(s: string): string {
  ...
  let out = String(s)
    ...
    .replace(/\s*\([^)]*\)/g, '')
    // Strip leading qty + unit. KEPT (product form distinctions):
    //   can/cans       (canned tomatoes vs fresh; canned beans vs dried)
    //   clove/cloves   (garlic clove vs garlic head/powder/puree)
    //   head           (head of garlic vs clove)
    // STRIPPED (true packaging — marinara is always jarred, butter is sold in sticks):
    //   jar, bottle, box, package, packet, carton, tube, bag, stick, bunch
    // Pass 1: strip leading quantity (digits + fractions + ranges)
    .replace(/^[\d/.½¼¾⅓⅔⅛⅜⅝⅞]+(?:[\s-][\d/.½¼¾⅓⅔⅛⅜⅝⅞]+)*\s+/i, '')
...
const np = StyleSheet.create({
  ...
  garnishItemRow:    { paddingVertical: 6, gap: 2 },
  garnishItemName:   { fontFamily: Fonts.bodyMedium, fontSize: 13, color: Colors.textPrimary },
  garnishItemMacros: { fontFamily: Fonts.body, fontSize: 12, color: Colors.textSecondary, lineHeight: 17 },
  garnishNoteFooter: { fontFamily: Fonts.body, fontSize: 10, color: Colors.textSecondary, fontStyle: 'italic', lineHeight: 14, marginTop: 8 },
