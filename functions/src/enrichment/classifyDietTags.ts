...
export interface DietTagResult {
  native: boolean;
  mod: boolean;
  notes: string;
}
...
function hitsOf(text: string, keywords: string[]): string[] {
...
export function classifyDietTags(
  ...
  const tags: Record<string, DietTagResult> = {};
  ...
  if (isVeganBlog) {
    ...
    const subs: string[] = [];
    if (hasEgg)   subs.push('replace the egg with a flax egg — mix 2 tablespoons ground flaxseed with 1 tablespoon water');
    if (hasDairy) { subs.push('use plant-based dairy alternatives'); subs.push('replace chicken broth with vegetable broth'); }
  ...
  if (!tags['Vg']) {
    if (isVegBlog) {
      ...
      const bgOnly = (
        containsAny(ingText, [...FISH_SAUCE_KW, ...CHICKEN_BROTH_KW, ...ANCHOVY_KW]) &&
        !MEAT_ING_KW
      ...
      if (bgOnly) {
        const subs: string[] = [];
        if (containsAny(ingText, FISH_SAUCE_KW))    subs.push('replace fish sauce with extra soy sauce and a squeeze of lime');
        if (containsAny(ingText, CHICKEN_BROTH_KW)) subs.push('replace chicken broth with vegetable broth');
        if (containsAny(ingText, ANCHOVY_KW))       subs.push('replace anchovy paste with 1 tablespoon tamari and 1 tablespoon capers with their juice');
  ...
  if (!tags['GF']) {
    if (!hasGluten) {
      ...
      if (onlySoySauce) {
        ...
        const subs: string[] = [];
        if (ingText.includes('soy sauce'))        subs.push('replace soy sauce with tamari');
        if (ingText.includes('oyster sauce'))     subs.push('replace oyster sauce with a GF variety');
        if (ingText.includes('hoisin sauce'))     subs.push('replace hoisin sauce with GF hoisin sauce');
        if (ingText.includes('worcestershire'))   subs.push('use GF Worcestershire sauce');
        ...
        if (containsAny(ingText, ['breadcrumbs', 'panko']))    subs.push('use GF panko');
        if (ingText.includes('pasta'))                          subs.push('replace pasta with a GF alternative — we like brown rice pasta for the most comparable texture');
        if (ingText.includes('orzo'))                           subs.push('use GF orzo such as cassava flour orzo');
        if (ingText.includes('couscous'))                       subs.push('replace couscous with GF couscous or cauliflower rice');
        ...
        if (containsAny(ingText, ['pita', 'naan']))             subs.push('use GF bread alternative');
        if (subs.length) tags['GF'] = { native: false, mod: true, notes: joinSentences(subs) };
      }
    }
  }
  ...
  if (!tags['DF']) {
    if (!hasDairy) {
      ...
      if (!dairyInTitle) {
        ...
        if (subs.length) tags['DF'] = { native: false, mod: true, notes: joinSentences(subs) };
      }
    }
  }
  ...
  if (!containsAny(ingText, KETO_DISQ_KW)) {
    ...
    if (!ketoInTitle) {
      const subs: string[] = [];
      if (easyHits.includes('honey'))                    subs.push('replace honey with a liquid allulose sweetener');
      ...
      if (ingText.includes('quinoa'))                    subs.push('serve over cooked vegetables instead of quinoa');
      if (ingText.includes('couscous'))                  subs.push('replace couscous with cauliflower rice');
      ...
      if (subs.length) tags['K'] = { native: false, mod: true, notes: joinSentences(subs) };
    }
  }

  // ── AIP — Autoimmune Protocol ────────────────────────────────────────────────
  const aipBlocks: Record<string, boolean> = {
  ...
  if (!activeBlocks.length) {
    ...
    if (aipBlocks['nightshade']) {
      if (containsAny(ingText, ['soy sauce', 'tamari']))    subs.push('replace soy sauce with coconut aminos');
      if (ingText.includes('tomato'))                        subs.push('omit tomatoes or replace with roasted beets or butternut squash for color');
      if (containsAny(ingText, ['bell pepper', 'pepper']))   subs.push('replace bell peppers with celery or zucchini');
      ...
      if (ingText.includes('curry powder'))                  subs.push('replace curry powder with turmeric');
    }
    if (aipBlocks['nut/seed']) {
      if (ingText.includes('soy sauce'))                     subs.push('replace soy sauce with coconut aminos');
      if (ingText.includes('miso'))                          subs.push('replace miso with coconut aminos');
      if (ingText.includes('fish sauce'))                    subs.push('replace fish sauce with coconut aminos');
      if (ingText.includes('vinegar'))                       subs.push('replace vinegar with fresh lime or lemon juice');
      if (containsAny(ingText, ['sesame', 'tahini']))        subs.push('remove sesame seeds and sesame oil');
    ...
    if (subs.length) tags['AIP'] = { native: false, mod: true, notes: joinSentences(subs) };
  }
  ...
  if (!containsAny(ingText, FODMAP_DISQ_KW)) {
    ...
    if (easyFodmap.length && !hardFodmap.length) {
      ...
      if (subs.length) tags['LF'] = { native: false, mod: true, notes: joinSentences(subs) };
    }
  }
  ...
  if (!containsAny(ingText, HISTAMINE_DISQ_KW)) {
    ...
    if (!hardHist.length) {
      ...
      if (subs.length) tags['LH'] = { native: false, mod: true, notes: joinSentences(subs) };
    }
  }
