...
function fetchHtml(url: string, timeoutMs = 12000): Promise<string> {
...
function fetchImageBytes(url: string, timeoutMs = 12000): Promise<{ bytes: Buffer; contentType: string }> {
  return new Promise((resolve, reject) => {
    const lib = url.startsWith('https') ? https : http;
    const req = lib.get(
      url,
...
function storageSlug(recipeId: string): string {
  return recipeId.replace(/[^a-z0-9-]/g, '-').replace(/-+/g, '-').slice(0, 80);
}

// ── Core: fetch og:image → upload to Storage → return public URL ─────────────

async function scrapeAndUpload(recipeId: string, pageUrl: string): Promise<string | null> {
  try {
    ...
    const blob  = bucket.file(`images/${slug}.${ext}`);
...
export const weeklyImageScrape = functions
...
export const scrapeImageForRecipe = functions.https.onCall(async (data, _context) => {
  ...
  if (!docSnap.exists) throw new functions.https.HttpsError('not-found', `Recipe ${recipeId} not found`);

  const pageUrl: string = docSnap.data()?.url || '';
  if (!pageUrl) throw new functions.https.HttpsError('failed-precondition', 'Recipe has no URL');

  const publicUrl = await scrapeAndUpload(recipeId, pageUrl);
  if (!publicUrl) return { success: false, recipeId };
